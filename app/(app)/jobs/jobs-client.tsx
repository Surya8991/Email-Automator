...
import { Plus, RefreshCw, Trash2, ExternalLink, Bookmark, Briefcase, CheckCircle2, XCircle, Building2 } from 'lucide-react'
...
function LeadsTable({ leads, pending, showSaveButton }: { leads: LeadRow[]; pending: boolean; showSaveButton: boolean }) {
  const router = useRouter()
  ...
  function setStatus(id: number, status: 'saved' | 'ignored' | 'applied') {
    start(async () => {
      await setJobLeadStatusAction(id, status)
